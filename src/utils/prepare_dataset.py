...
def run_split():
    ...
    if CONFIG["dest_dir"].exists():
        warnings.warn(
            f"Destination directory '{CONFIG['dest_dir']}' already exists. Files may be overwritten."
        )
    ...
    for class_name in CONFIG["classes"]:
        ...
        if not source_class_dir.exists():
            print(
                f"Warning: Source directory not found for class '{class_name}'. Skipping."
            )
            continue
        ...
        validation_end = train_end + int(
            CONFIG["split_ratios"]["validation"] * num_images
        )
        splits = {
